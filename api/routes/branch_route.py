...
def create_branch(branch: BranchSchema, id: int = Depends(get_current_user), db: Session = Depends(get_db)):
    check_permission_role_supper_admin(id=id, db=db)
    branch = BranchRepository.find_by_name(db, branch.branch_name)
    if branch:
        return ResponseSchema.from_api_route(status_code=400, data="Branch already exist").dict(exclude_none=True)
    branch = BranchRepository.insert(db, Branch(**branch.dict()))
    return ResponseSchema.from_api_route(status_code=201, data=branch).dict(exclude_none=True)


@branch_r.put("/{branch_id}", dependencies=[Depends(JWTBearer())], response_model=ResponseSchema[BranchSchema])
def update_branch(branch_id: int, branch: BranchSchema, id: int = Depends(get_current_user),
...
@branch_r.delete("/{branch_id}", dependencies=[Depends(JWTBearer())], response_model=ResponseSchema[BranchSchema])
